...
private:
    ...
    Query ParseQuery(const std::string_view& text, bool policy = false) const;
    ...
    template<typename Predicate, class Execution>
    std::vector<Document> FindAllDocuments(Execution&& policy, const Query& query, Predicate predicate) const;
...
std::vector<Document> SearchServer::FindTopDocuments(Execution&& policy, const std::string_view& raw_query, DocumentPredicate document_predicate) const {
    if  (std::is_same_v<std::decay_t<Execution>, std::execution::sequenced_policy>) {
...
template<typename Predicate, class Execution>
std::vector<Document> SearchServer::FindAllDocuments(Execution&& policy, const Query& query, Predicate predicate) const {
    ...
    std::vector<Document> matched_documents;
    matched_documents.resize(ordinaryMap.size());
